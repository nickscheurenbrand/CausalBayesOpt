...
#PBS -N oracle_gwps_spherical_rbf_ei

source $HOME/causal_bayes_opt/.venv/bin/activate
cd $HOME/causal_bayes_opt/scripts_gwps
mkdir -p $HOME/causal_bayes_opt/results/boundary_tracking_gwps_oracle_spherical_rbf

# ORACLE GWPS gene network (60-node DAG, linear G_hat SEM at weight_scale=3)
# boundary tracking (intervene only on the true parents) with the spherical-RBF
# surrogate kernel, EI acquisition only, then the boundary-bias analysis.
# Assumes data/gwps_direct_edges.csv exists.
KERNEL=spherical_rbf; SUBDIR=boundary_tracking_gwps_oracle_spherical_rbf
