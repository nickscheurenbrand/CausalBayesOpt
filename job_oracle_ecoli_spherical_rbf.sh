...
#PBS -N oracle_ecoli_spherical_rbf_ei

source $HOME/causal_bayes_opt/.venv/bin/activate
cd $HOME/causal_bayes_opt/scripts_dream
mkdir -p $HOME/causal_bayes_opt/results/boundary_tracking_dream_oracle_spherical_rbf

# ORACLE Ecoli Size50 + Size100 (DREAM, nonlinear SEM) boundary tracking
# (intervene only on the true parents) with the spherical-RBF surrogate kernel,
# EI acquisition only, then the boundary-bias analysis.
KERNEL=spherical_rbf; SUBDIR=boundary_tracking_dream_oracle_spherical_rbf
