r"""Adaptive intervention geometry (appendix E.3). Reshapes the intervention space by the parent
posterior, x~ = phi_pi(x) = S(diag(pi_I) x), so coordinates believed causal keep their extent."""
...
def marginal_parent_probabilities(
    ...
    """Marginal P(v is a causal parent | D_t) for every variable in the support. Accepts either
    a {parent_set: probability} mapping or a (parent_sets, probabilities) pair."""
...
class AdaptiveGeometry:
    r"""x -> x~ = S(W(pi) (x - c)) and the induced feature map psi(x~). ``pi_floor`` clips pi so
    a zero-probability coordinate is contracted but not annihilated."""
