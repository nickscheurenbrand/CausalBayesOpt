...
#PBS -N oracle_erdos_spherical_rbf_ei

source $HOME/causal_bayes_opt/.venv/bin/activate
cd $HOME/causal_bayes_opt/scripts_erdos
mkdir -p $HOME/causal_bayes_opt/results/boundary_tracking_oracle_spherical_rbf

# ORACLE Erdos50 + Erdos100 boundary tracking (intervene only on the true
# parents) with the spherical-RBF surrogate kernel, EI acquisition only, then
# the boundary-bias analysis.
KERNEL=spherical_rbf; SUBDIR=boundary_tracking_oracle_spherical_rbf
