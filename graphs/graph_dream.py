...
class Dream4Graph(GraphStructure):
    ...
    def __init__(
        ...
        self.causal_env: CausalEnvironment = OnlyDAGDream4Environment(
            args=args, name=yml_name, nonlinear=True, seed=seed
        )
        ...
        print(self.causal_env.nodes)

        self.num_nodes = len(self.causal_env.nodes)
        self._variables = [str(i) for i in range(self.num_nodes)]
        self._nodes = sorted(set(self.variables))

        self._parents, self._children = self.build_relationships()
        self._target = f"{self.num_nodes - 1}"
